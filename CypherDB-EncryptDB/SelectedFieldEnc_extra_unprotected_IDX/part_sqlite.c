...
{
  char *partTable = "create table Part(p_partKey INTEGER, p_name CHAR(55), p_mfgr CHAR(25), p_brand CHAR(10), p_type CHAR(25), p_size INTEGER, p_container CHAR(10), p_retailprice REAL, p_comment CHAR(23), p_partKeyIDX INTEGER);";
...
void *partTemplateGen(char *directory, float scale)
{
  ...
  count = PARTMAX*scale;
  part_t = (part *)calloc(1, sizeof(part));
  for (i=0; i<count; i++)
    {
      readPart(part_t, ifp);
