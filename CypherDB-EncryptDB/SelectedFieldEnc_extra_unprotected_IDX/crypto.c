...
void *BytesFromIntv2(unsigned char *result, unsigned int value, unsigned int noBytes)
{
...
void * ctrEncryptINT(int input, int column, char *cipher, int len, unsigned char counter[AES_BLOCK_SIZE], const AES_KEY * aes)
{
	...
	plain = (char *)calloc(16, sizeof(unsigned char));

	BytesFromIntv2(plain, input,16);
#ifdef debug_int
