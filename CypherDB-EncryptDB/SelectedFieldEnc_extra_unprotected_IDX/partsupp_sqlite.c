...
{
  char *partSuppTable = "create table PartSupp(ps_partkey INTEGER, ps_suppkey INTEGER, ps_availqty INTEGER, ps_supplycost REAL, ps_comment CHAR(199), ps_partkeyIDX INTEGER, ps_suppkeyIDX INTEGER);";
...
void *partSuppTemplateGen(char *directory, float scale)
{
  ...
  partSupp_t = (partSupp *)calloc(1, sizeof(partSupp));
  for (i=0; i<count; i++)
    {
      readPartSupp(partSupp_t, ifp);
      printPartSuppTemplate(partSupp_t, tfp);
      freePartSupp(partSupp_t);
    }
  free(partSupp_t);
  fclose(ifp);
  fclose(tfp);
