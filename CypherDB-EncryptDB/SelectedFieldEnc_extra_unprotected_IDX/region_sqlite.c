...
{
  char *regionTable = "create table Region(r_regionkey INTEGER, r_name CHAR(25),  r_comment CHAR(152), r_regionkeyIDX INTEGER);";
...
void *regionTemplateGen(char *directory, float scale)
{
  ...
  count = REGIONMAX*1;
  regior_t = (region *)calloc(1, sizeof(region));
  for (i=0; i<count; i++)
    {
