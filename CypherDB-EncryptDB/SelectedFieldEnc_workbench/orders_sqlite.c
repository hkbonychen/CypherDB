...
{
    char *ordersTable = "create table Orders(o_orderKey INTEGER, o_custKey INTEGER, o_orderStatus CHAR(1) NOT NULL, o_totalPrice REAL, o_orderDate DATETIME, o_orderPriority CHAR(15) NOT NULL, o_clerk CHAR(15) NOT NULL, o_shipPriority INTEGER, o_comment CHAR(79) NOT NULL, o_orderKeyOPE INTEGER, o_custKeyOPE INTEGER);";
...
void *ordersTemplateGen(char *directory, float scale)
{
    ...
    orders_t = (orders *)calloc(count, sizeof(orders));
    for (i=0; i<count; i++)
    {
        readOrders(&orders_t[i], ifp);
        printOrdersTemplate(&orders_t[i], tfp);
    }
    fclose(ifp);
    fclose(tfp);
}
