...
{
  char *supplierTable = "create table Supplier(s_suppkey INTEGER, s_name CHAR(25), s_address CHAR(40), s_nationkey INTEGER, s_phone CHAR(15), s_acctbal REAL, s_comment CHAR(101), s_suppkeyOPE INTEGER, s_nationkeyOPE INTEGER);";
...
void *supplierTemplateGen(char *directory, float scale)
{
  ...
  supplier_t = (supplier *)calloc(count, sizeof(supplier));
  for (i=0; i<count; i++)
    {
      readSupplier(&supplier_t[i], ifp);
      printSupplierTemplate(&supplier_t[i], tfp);
    }
  fclose(ifp);
  fclose(tfp);
}
