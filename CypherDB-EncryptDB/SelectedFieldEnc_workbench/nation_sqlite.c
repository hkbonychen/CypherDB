...
{
  char *nationTable = "create table Nation(n_nationkey INTEGER, n_name CHAR(25), n_regionkey INTEGER, n_comment CHAR(152), n_nationkeyOPE INTEGER, n_regionkeyOPE INTEGER);";
...
void *nationTemplateGen(char *directory, float scale)
{
  ...
  count = NATIONMAX*1;
  nation_t = (nation *)calloc(count, sizeof(nation));
  for (i=0; i<count; i++)
    {
      readNation(&nation_t[i], ifp);
      printNationTemplate(&nation_t[i], tfp);
    }
  fclose(ifp);
  fclose(tfp);
}
