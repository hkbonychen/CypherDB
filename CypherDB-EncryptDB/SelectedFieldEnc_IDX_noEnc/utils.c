...
unsigned char *
BytesFromIntCount(int value, unsigned int count)
{
    ...
    len = count;
    result = (char *)calloc(count, sizeof(unsigned char));
    
    while( len-- ){
      result[len] = (char )(value&0xFF);
      value >>= 8;
    }

//    printHex(stdout, result, count);
    return result;
}
...
void incr_row_128(unsigned char bitstream[AES_BLOCK_SIZE], int value)
{
	...
	counter = *(int *)(&(temp2[0]));
	counter+=value;
	BytesFromInt(&temp_buf, counter, 4); 

	for (j=0; j<4; ++j) temp[j] = temp_buf[j];
	free(temp_buf);
