...
unsigned int
payloadHdr(unsigned int col, columnDataType *dataType)
{
...
unsigned int getPageData(unsigned char **pageBuffer, int Offset, int Count, unsigned int * rowID, unsigned int * startingAddr, unsigned int * endAddr, char *encArr, int columnNum, int **size)
{
	int i, bytesRead;
   	...
   	for (i=0;i<columnNum;i++)
	{
		...
		(*size)[i] = payloadHdr(columnLen[i], &columnType[i]);
		//printf("size = %d \n", (*size)[i]);
		if (encArr[i]) {
		  switch (columnLen[i])
		  {
