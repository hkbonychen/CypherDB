...
{
  char *nationTable = "create table Nation(n_nationkey INTEGER, n_name CHAR(25), n_regionkey INTEGER, n_comment CHAR(152));";
  ...
  for (i=0; i<count; i++)
    {
      ...
      nation_free(&nation_t);
    }
  fclose(ifp);
  fclose(tfp);
}

unsigned int modifyPageDataNation(unsigned char **pageBuffer, unsigned int recordOffset, nation* sizePt, nationOTP* nationOTPPt, char *encArr, unsigned int *cellsize)
{
  unsigned int  cursor;
  ...
  /* size [0]*/
	if (sizePt->nationKey > 1) {
  		temp = BytesFromIntCount(sizePt->nationKey, cellsize[0]);		
  ...
  // size [1]
  memcpy(&((*pageBuffer)[cursor]), nationOTPPt->name, cellsize[1]);
  cursor += cellsize[1];

  // size [2]
	if (sizePt->regionKey > 1) {
  		temp = BytesFromIntCount(sizePt->regionKey, cellsize[2]);		
  ...
  // size [3]
  memcpy(&((*pageBuffer)[cursor]), nationOTPPt->comment, cellsize[3]);
  cursor += cellsize[3];
  ...
  while (pageCount<maxPageCount_t)
    {
	  ...
	  cellOffset = getPageData(&pageBuffer, cellOffset, cellCount, &(recordPagewiseAddr->recordRowID[i]), &(recordPagewiseAddr->recordStartingAddr[i]), &(recordPagewiseAddr->recordEndAddr[i]),encArray, NATIONNCOL, &cellsize);
	  /* modify the pageBuffer by storing back the encrypted data in itemOTP */
	  modifyPageDataNation(&pageBuffer, recordPagewiseAddr->recordStartingAddr[i], &(nationPt[recordPagewiseAddr->recordRowID[i]-1]), &(nationOTPPt[recordPagewiseAddr->recordRowID[i]-1]), encArray, cellsize);
	  free(cellsize);
        }
