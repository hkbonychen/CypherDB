...
void Start()
{
  ...
  unsigned long b=0x0001;
   ...
   uart_init();
   uart_print_str("start sdc testing ... \n");
   ...
   sd_card_0 = sd_controller_init(); 
    if (sd_card_0.Active==1)
	{
		...
		uart_print_str("\nHCS? :\n");
		uart_print_long(sd_card_0.HCS_s);
		uart_print_str(":\n");
	}
	else
        ...
        SD_REG(SD_COMMAND) = CMD9  |WORD_0| CICE | CRCE | RSP_136;  
		//SD_REG(SD_ARG)=sd_card_0.rca | 0xf0f0;
	SD_REG(SD_ARG)=sd_card_0.rca;

           if (!sd_wait_rsp())
			 	uart_print_str(" send failed :/!\n");
		 else{
		 	  uart_print_str("Card_Capacity: \n");
	...
	//Set Bus width to 4, CMD55 followed by ACMD 6
 		...
 		REG32(SD_CONTROLLER_BASE+SD_ARG) =sd_card_0.rca;
	 		if (!sd_wait_rsp())
		...
		REG32(SD_CONTROLLER_BASE+SD_CLOCK_D)=0x0000004;

	// write data 

		/* uart_print_str("block address to write: \n"); */
		/* uart_print_long(&block);		 */
		/* uart_print_str("\n");  */
	...
	// read data
		...
		uart_print_str("block address to be read: \n");
		uart_print_long(&rec_block);
		uart_print_str("\n"); 

		SD_REG(BD_RX)  = &rec_block;
		SD_REG(BD_RX)  = 1536;
	       
		uart_print_str("BD_RX transfer \n");
		...
		uart_print_str("BD_RX transfer done \n");
